 ...
 *
 * 每个记录器Logger的属性:
 *  name 属性：记录器的名称。记录器的唯一表示(eg: com.mumu.logback.Demo1)
 *  level属性（可选）：记录器的级别，允许的级别从低到高，TRACE＜DEBUG ＜INFO ＜ WARN＜ERROR
 *      logger.setLevel();  设置日志级别
 *      logger.getLevel();  查看当前日志级别
 *      logger.getEffectiveLevel() 查看实际有效的记录仪
 *      additivity属性（可选）：是否允许叠加打印日志，true或false
 *  说明：
 *      1. 如果记录器未设置level属性，则该记录器的级别从上级记录器继承
 *      2. 如果想查看记录器的级别，应该通过 logger.getEffectiveLevel() 方法，同时要将Logger转为 ch.qos.logback.classic.Logger类型，如下：
 *      ch.qos.logback.classic.Logger logger = (ch.qos.logback.classic.Logger)LoggerFactory.getLogger(Demo1.class);
 *
 *
 * 附加器Appender
 * 什么是附加器？
 ...
 *
 * %class{length}  日志请求的调用者的完全限定类名
 * %line          输出发出记录请求的行号
 * date{pattern}  date{pattern,timezone}  输出日志记录事件的日期
 *  eg:
 ...
 *
 * 常用的过滤器
 *  LevelFilter（级划过滤器)：实现类 ch.qos.logback.classic.filter.LevelFilter
 ...
 *  JaninoEventEvaluator (过滤器)：实现类 ch.qos.logback.core.filter.EvaluatorFilter
 *  TurboFilter (涡轮过滤器)
